...
export const connectionAccessTypeOptions = [
  {
    label: formatMessage({
      id: 'odc.components.FormAutoAuthModal.PublicConnection',
    }), //公共连接
...
const FormAutoAuthModal: React.FC<IProps> = (props) => {
  ...
  const [connectionAccessOptionsMap, setConnectionAccessOptionsMap] = useState<{
    ...
    [IManagerResourceType.public_connection]: getResourceOptions(publicConnections?.contents),
  });
  ...
  const roleOptions = [...roles.values()]?.map((item) => ({
    label: item.name,
    value: item.id,
  }));
...
export default FormAutoAuthModal;
