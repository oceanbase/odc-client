...
import { PermissionTypes, UserDetailContent } from './component';
...
const authTypeFilter = [
  ...
  {
    text: formatMessage({ id: 'odc.components.RolePage.ConnectionAccess' }), //连接访问权限
...
class RolePage extends React.PureComponent<IProps, IState> {
  ...
  private getPageColumns = () => {
    return [
      ...
      {
        title: formatMessage({ id: 'odc.components.RolePage.State' }), // 状态
        width: 115,
        ellipsis: true,
        key: 'enabled',
        dataIndex: 'enabled',
  ...
  private loadData = async () => {
    await this.context.getRoleList();
  };

  private handleCloseAndReload = () => {
    this.handleCloseDetailModal();
    this.loadData();
    this.context.getRoleList();
  };
  ...
  render() {
    const { formModalVisible, detailModalVisible, editId, detailId, copyId, loading } = this.state;
    ...
    return (
      <>
        <CommonTable
          ...
          titleContent={{
            title: formatMessage({
              id: 'odc.components.RolePage.RoleManagement',
            }),
          ...
          tableProps={{
            ...
            dataSource: this.handleFilterAndSort([...roles.values()]),
            rowKey: 'id',
          }}
        />
