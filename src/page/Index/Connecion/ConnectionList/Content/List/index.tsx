import { batchTest, cancelConnectionTop, setConnectionTop } from '@/common/network/connection';
import { IConnection, IConnectionStatus, IConnectionType } from '@/d.ts';
import { Checkbox, message, Result, Space, Spin, Tooltip } from 'antd';
import React, {
  forwardRef,
  useContext,
  useEffect,
...
import PinActiveSvg from '@/svgr/icon_pin_filled.svg';
import PinSvg from '@/svgr/icon_pin_outlined.svg';
...
const List: React.FC<IProps> = forwardRef(function (
  { width, height, connectionStore, schemaStore, commonStore, pageStore, clusterStore },
  ref,
) {
  ...
  const { searchValue, connectType, permissions, sortType, visibleScope, label } = context;
  ...
  async function openNewConnection(connection: IConnection) {
    ...
    try {
      ...
      if (connectionStore.sessionId) {
        ...
        if (connectionStore.connection.defaultSchema) {
          schemaStore.database = {
            name: connectionStore.connection.defaultSchema,
          };
        }
  ...
  async function _fetchNextConnectList(isRefresh?: boolean) {
    ...
    try {
      ...
      const result = await connection.getList({
        ...
        size: fetchSize,
        visibleScope:
          visibleScope === IConnectionType.ALL ? null : visibleScope || IConnectionType.PRIVATE,
        permittedAction: permissions,
        sessionLabelId: label,
      });
  ...
  }, [searchValue, connectType, permissions, sortType, visibleScope, label]);
    ...
    if (index >= connectionList?.length) {
      ...
      content = (
        <ListItem
          ...
          checkbox={
            <Checkbox
              checked={context.selectedKeys.has(connection.id)}
              disabled={connection.visibleScope === IConnectionType.ORGANIZATION}
              onChange={(e) => {
                const checked = e.target.checked;
                let newSet = new Set(context.selectedKeys);
                if (!checked) {
                  newSet.delete(connection.id);
                } else {
                  newSet.add(connection.id);
                }
                context.setSelectedKeys(newSet);
              }}
            />
          }
          connectionName={
          ...
          action={
            <Space size={14}>
              <Tooltip
                title={
                  connection.setTop
                    ? formatMessage({ id: 'odc.Content.List.CancelTheTop' }) //取消置顶
                    : formatMessage({ id: 'odc.Content.List.Top' }) //置顶
                }
              >
                <IconBtn
                  icon={PinSvg}
                  activeIcon={PinActiveSvg}
                  isActive={connection.setTop}
                  onClick={async () => {
                    let isSuccess;
                    if (connection.setTop) {
                      isSuccess = await cancelConnectionTop(connection.id);
                    } else {
                      isSuccess = await setConnectionTop(connection.id);
                    }
                    isSuccess && (await _fetchNextConnectList(true));
                  }}
                />
              </Tooltip>
              <LabelBtn connection={connection} />
