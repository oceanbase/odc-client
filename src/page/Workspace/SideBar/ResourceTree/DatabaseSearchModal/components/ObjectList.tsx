...
const ObjectList = ({ modalStore }: Iprops) => {
    ...
    typeList?.forEach((i) => {
      switch (i) {
        ...
        case DbObjectType.synonym:
          // 同义词在全部tab下分开展示为普通同义词和公共同义词
          const synonyms = objectlist?.dbObjects?.filter(
            (obj) => obj.type === DbObjectType.synonym,
          );
          const publicSynonyms = objectlist?.dbObjects?.filter(
            (obj) => obj.type === DbObjectType.public_synonym,
          );

          if (synonyms?.length > 0) {
            typeObjectTree.push({ key: DbObjectType.synonym, data: synonyms });
          }
          if (publicSynonyms?.length > 0) {
            typeObjectTree.push({ key: DbObjectType.public_synonym, data: publicSynonyms });
          }
          break;
        default:
          typeObjectTree.push({
  ...
  const renderObjectTypeTabs = (type) => {
    ...
    const isSynonymTab = currentObjectList?.key === DbObjectType.synonym;

    // 如果是同义词tab，需要分组展示
    if (isSynonymTab) {
      const synonyms =
        currentObjectList?.data?.filter((obj) => obj.type === DbObjectType.synonym) || [];
      const publicSynonyms =
        currentObjectList?.data?.filter((obj) => obj.type === DbObjectType.public_synonym) || [];
        ...
        <Spin spinning={objectloading}>
          {!currentObjectList?.data?.length ? (
            ...
            <div className={styles.objectlistBox}>
              ...
              {synonyms.length > 0 && (
                <div className={styles.objectTypeBox}>
                  <div className={styles.objectTypeTitle}>
                    {DbObjectTypeMap[DbObjectType.synonym]?.label}
                  </div>
                  <div>
                    {synonyms.map((object) => (
                      <div
                        ...
                        onClick={(e) => {
                          let params = {
                            ...
                            database: object.database as IDatabase,
                            name: object.name,
                            objectName: undefined,
                          };
                          positionResourceTree?.(params);
                      ...
                      >
                        <div style={{ overflow: 'hidden', display: 'flex', width: '100%' }}>
                          <Icon
                            component={DbObjsIcon[DbObjectType.synonym]}
                            style={{
                              color: 'var(--brand-blue6-color)',
                              paddingRight: 4,
                              fontSize: 14,
                          ...
                          >
                            {getSubTitle(object, DbObjectType.synonym)}
                          </span>
                        </div>
                        {permissionBtn(object, DbObjectType.synonym)}
                      </div>
                    ))}
                  </div>
              ...
              {/* 公共同义词分组 */}
              {publicSynonyms.length > 0 && (
                <div className={styles.objectTypeBox}>
                  <div className={styles.objectTypeTitle}>
                    {DbObjectTypeMap[DbObjectType.public_synonym]?.label}
                  </div>
                  <div>
                    {publicSynonyms.map((object) => (
                      <div
                        key={object.id}
                        className={styles.objectItem}
                        onClick={(e) => {
                          let params = {
                            type: DbObjectType.public_synonym,
                            database: object.database as IDatabase,
                            name: object.name,
                            objectName: undefined,
                          };
                      ...
                      >
                        <div style={{ overflow: 'hidden', display: 'flex', width: '100%' }}>
                          <Icon
                            component={DbObjsIcon[DbObjectType.public_synonym]}
                            style={{
                              color: 'var(--brand-blue6-color)',
                              paddingRight: 4,
                              fontSize: 14,
                          ...
                          >
                            {getSubTitle(object, DbObjectType.public_synonym)}
                          </span>
                        </div>
                        {permissionBtn(object, DbObjectType.public_synonym)}
                      </div>
                    ))}
                  </div>
                  {publicSynonyms.length === MAX_OBJECT_LENGTH && (
                    <Divider plain>
