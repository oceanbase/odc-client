...
export function NoSpaceEditor<T>({ row, onRowChange, column, left, top, width }: EditorProps<T>) {
  ...
  const innerOnChange = useCallback(
    (e: ChangeEvent<HTMLTextAreaElement>) => {
      ...
      onRowChange({ ...row, [key]: filteredValue });
    },
    [onRowChange, row, key],
  );

  const modalOnChange = useCallback((e: ChangeEvent<HTMLTextAreaElement>) => {
    const filteredValue = valueFilter(e.target.value);
    setModalTextValue(filteredValue);
  }, []);
