...
import { getItems as getAsyncTaskContentItems } from '@/component/AsyncTaskModal';
import CommonTaskDetailModal from '@/component/CommonTaskDetailModal';
import DataTransferTaskContent from '@/component/DataTransferModal';
...
import { getItems as getDataMockerItems } from './dataMocker';
import PartitionTaskContent from './partition';
import { getItems as getPermissionItems } from './permission';
import { getItems as getShadowSyncItems } from './shadowSync';
import SqlPlanTaskContent from './sqlPlan';
...
const DetailModal: React.FC<IProps> = React.memo((props) => {
  ...
  const hasLog = task && task?.type !== TaskType.PERMISSION_APPLY;
  const hasResult =
    ![TaskType.PERMISSION_APPLY, TaskType.ALTER_SCHEDULE].includes(type) &&
  ...
  const modalProps = {
    ...
    logType,
    isLoading: loading,
    isSplit: ![TaskType.ASYNC, TaskType.PERMISSION_APPLY].includes(task?.type),
    taskTools: (
      <TaskTools
        isDetailModal
