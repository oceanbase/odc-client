...
const getLogicalDatabaseAsyncColumns = (params: {
  handleLogicalDatabaseAsyncModalOpen: (data: sqlExecutionResultMap) => void;
  ...
  return [
    {
      ...
      onFilter: (value, record: sqlExecutionResultMap) => {
        return record?.physicalDatabase?.name?.includes(value);
      },
      render: (physicalDatabase: IDatabase, record: sqlExecutionResultMap) => {
    ...
    },
    {
      ...
      render: (value, record: sqlExecutionResultMap) => {
        return record?.physicalDatabase?.dataSource?.name ?? '-';
      },
      filterDropdown: (props) => {
        return (
          <SearchFilter
      ...
      onFilter: (value, record: sqlExecutionResultMap) => {
        return record?.physicalDatabase?.dataSource?.name?.includes(value);
      },
    },
    {
      title: formatMessage({
      ...
      key: 'status',
      dataIndex: 'status',
      render: (value, row: sqlExecutionResultMap) => {
        return (
          <Space>
    ...
    },
    {
      ...
      render: (value, record: sqlExecutionResultMap) => {
        return (
          <Space>
            <Link onClick={() => params?.handleLogicalDatabaseAsyncModalOpen(record)}>
              {formatMessage({
            ...
            {record?.status === SchemaChangeRecordStatus.RUNNING && (
              <Link onClick={() => params?.handleLogicalDatabaseTaskStop(record)}>
                {formatMessage({
            ...
            {[
              ...
              <Link onClick={() => params?.handleLogicalDatabaseTaskSkip(record)}>
                {formatMessage({
