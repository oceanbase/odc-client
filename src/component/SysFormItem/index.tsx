...
const SysFormItem: React.FC<IProps> = inject('connectionStore')(
  observer(function (props) {
    const { connectionStore, form, enforce, tip } = props;
    ...
    const sysUser = connectionStore.connection?.sysUser;
    const sid = connectionStore.connection?.sid;
    ...
    async function testSys() {
      const connection = cloneDeep(connectionStore.connection);
      if (!connection) {
        return;
      }
      ...
      if (!res?.data?.active) {
        ...
        setStatus('success');
        message.success(
          formatMessage({ id: 'odc.component.SysFormItem.ConnectionSucceeded' }), // 连接成功
        );
      }
    }
              ...
              <>
                <Row gutter={16}>
                  ...
                  <Col span={12}>
                    ...
                    {!pwdEdit ? (
                      <Form.Item name="sid" style={{ display: 'none' }}>
                        <Input />
                      </Form.Item>
                    ) : null}
                  </Col>
